...
Kling O1 via OpenRouter — first_frame + last_frame, 10s, ~$0.90/run.
...
def _img_to_jpeg_b64(path: str, target_size: int = 1280) -> tuple[str, str]:
...
class VideoPipeline:
    ...
    async def generate_pool_video(
        ...
        """
        Generate a 10-second pool appearance video using Kling O1 (first + last frame).
        Raises RuntimeError on any failure — no silent fallback.
        """
        start_b64, start_mime = _img_to_jpeg_b64(satellite_path)
        end_b64,   end_mime   = _img_to_jpeg_b64(render_path)
        ...
        print(f"  [Kling O1] job {job_id} submitted")
